...
const internals_opendata_feedback = {
    sqlBaseView: Knex.select('*').from('tb_opendata_feedback')
};
...
const internals_satupeta_bantuan = {
    sqlBaseView: Knex.select('*').from('tb_satupeta_bantuan')
};
const internals_satupeta_feedback = {
    sqlBaseView: Knex.select('*').from('tb_satupeta_feedback')
};
...
const internals_log = {
    sqlBaseView: Knex.select('*').from('tb_logging')
};
    ...
    view_count: async function (request, h) {
        ...
        try {
            ...
            /*  const count_satupeta_locations_raw = await Knex('tb_satupeta_locations')
                ...
                .first(); */
            let countQuerysatupeta_locations = Knex('tb_satupeta_locations')
                .count('id_location as count_data');

            if (search_satker?.trim()) {
                countQuerysatupeta_locations = countQuerysatupeta_locations.where(
                    'tb_satupeta_locations.satker_id',
                    search_satker.trim()
                );
            }

            const count_satupeta_locations_raw = await countQuerysatupeta_locations.first();

            const count_satupeta_locations = count_satupeta_locations_raw
                ? parseInt(count_satupeta_locations_raw.count_data, 10)
                : 0;

            
            let countQuerysatupeta_location_maplist = Knex('tb_satupeta_location_maplist')
                .count('id_maplist as count_data');

            if (search_satker?.trim()) {
                countQuerysatupeta_location_maplist = countQuerysatupeta_location_maplist.where(
                    'tb_satupeta_location_maplist.satker_id',
                    search_satker.trim()
                );
            }

            const count_satupeta_location_maplist_raw = await countQuerysatupeta_location_maplist.first();

            const count_satupeta_location_maplist = count_satupeta_location_maplist_raw
                ? parseInt(count_satupeta_location_maplist_raw.count_data, 10)
                : 0;
 ...
 */
            let countQuerysatupeta_location_maplist_download = Knex('tb_satupeta_location_maplist_download')
                .join('tb_satupeta_location_maplist', 'tb_satupeta_location_maplist_download.id_maplist', '=', 'tb_satupeta_location_maplist.id_maplist')
                .select(
                    'tb_satupeta_location_maplist.satker_id as satker_id'
                )
                .count('tb_satupeta_location_maplist_download.id as count_data');

            if (search_satker?.trim()) {
                countQuerysatupeta_location_maplist_download = countQuerysatupeta_location_maplist_download.where(
                    'tb_satupeta_location_maplist.satker_id',
                    search_satker.trim()
                );
            }

            const count_satupeta_location_maplist_download_raw = await countQuerysatupeta_location_maplist_download.first();

            const count_satupeta_location_maplist_download = count_satupeta_location_maplist_download_raw
                ? parseInt(count_satupeta_location_maplist_download_raw.count_data, 10)
                : 0;
 ...
 */
            ...
            let countQuerysatupeta_location_maplist_visitors = Knex('tb_satupeta_location_maplist_visitors')
                .join('tb_satupeta_location_maplist', 'tb_satupeta_location_maplist_visitors.id_maplist', '=', 'tb_satupeta_location_maplist.id_maplist')
                .select(
                    'tb_satupeta_location_maplist.satker_id as satker_id'
                )
                .count('tb_satupeta_location_maplist_visitors.id as count_data');

            if (search_satker?.trim()) {
                countQuerysatupeta_location_maplist_visitors = countQuerysatupeta_location_maplist_visitors.where(
                    'tb_satupeta_location_maplist.satker_id',
                    search_satker.trim()
                );
            }

            const count_satupeta_location_maplist_visitors_raw = await countQuerysatupeta_location_maplist_visitors.first();

            const count_satupeta_location_maplist_visitors = count_satupeta_location_maplist_visitors_raw
                ? parseInt(count_satupeta_location_maplist_visitors_raw.count_data, 10)
                : 0;
 ...
 */
            ...
            let countQuerysatupeta_location_points = Knex('tb_satupeta_location_points')
                .join('tb_satupeta_location_maplist', 'tb_satupeta_location_points.location_id', '=', 'tb_satupeta_location_maplist.location_id')
                .select(
                    'tb_satupeta_location_maplist.satker_id as satker_id'
                )
                .count('tb_satupeta_location_points.id_location_point as count_data');

            if (search_satker?.trim()) {
                countQuerysatupeta_location_points = countQuerysatupeta_location_points.where(
                    'tb_satupeta_location_maplist.satker_id',
                    search_satker.trim()
                );
            }

            const count_satupeta_location_points_raw = await countQuerysatupeta_location_points.first();

            const count_satupeta_location_points = count_satupeta_location_points_raw
                ? parseInt(count_satupeta_location_points_raw.count_data, 10)
                : 0;

            let countQuerysatupeta_location_geospasial = Knex('tb_satupeta_location_geospasial')
                .join('tb_satupeta_location_maplist', 'tb_satupeta_location_geospasial.id_maplist', '=', 'tb_satupeta_location_maplist.id_maplist')
                .select(
                    'tb_satupeta_location_maplist.satker_id as satker_id'
                )
                .count('tb_satupeta_location_geospasial.id_location_geospasial as count_data');

            if (search_satker?.trim()) {
                countQuerysatupeta_location_geospasial = countQuerysatupeta_location_geospasial.where(
                    'tb_satupeta_location_maplist.satker_id',
                    search_satker.trim()
                );
            }

            const count_satupeta_location_geospasial_raw = await countQuerysatupeta_location_geospasial.first();

            const count_satupeta_location_geospasial = count_satupeta_location_geospasial_raw
                ? parseInt(count_satupeta_location_geospasial_raw.count_data, 10)
                : 0;
            ...
            return h.response({
                ...
                count_satupeta_location_points,count_satupeta_location_geospasial,count_satupeta_visitors
            }).code(200);
    ...
    view_ekosistem_menu: async function (request, h) {
        try {
            ...
            const datas = await Knex('tb_satuportal_menu')
                .select('category')
                .groupBy('category')
    ...
    view_ekosistem2_menu: async function (request, h) {
        ...
        try {
            ...
            if (queryUrl.categoryku) {
                sql.where('category', 'like', `%${queryUrl.categoryku}%`);
            }
    ...
    view_ekosistem_iklan: async function (request, h) {
        ...
        // --- Ambil 1 data random ---
            ...
            sqlLatest.where('visibilitas', 'Publik');
    ...
    add_ekosistem_iklan: async function (request, h) {
        const { kategori, title, linked, visibilitas,admin,jenis,komponen } = request.payload;
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Tambah data Iklan aplikasi Satu Portal',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_ekosistem_iklan: async function (request, h) {
        ...
        const { title, linked, visibilitas,admin,jenis,komponen } = request.payload;
        const file = request.payload.file;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Update data Iklan aplikasi Satu Portal',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    delete_ekosistem_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: data.title,
                deskripsi: 'Delete data Iklan aplikasi Satu Portal',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_ekosistem_bio: async function (request, h) {
        ...
        const {
            email,telpon,fax,alamat,instagram,linkedin,facebook,whatapp,twitter
        } = request.payload;
    ...
    view_ekosistem_list_id: async function (request, h) {
        try {
            ...
            const images = [
                ...
                { key: 'logo_a', urlKey: 'presignedUrl_logo_a' },
                { key: 'logo_b', urlKey: 'presignedUrl_logo_b' },
                { key: 'logo_c', urlKey: 'presignedUrl_logo_c' }
            ];
    ...
    view_opendata_menu: async function (request, h) {
        try {
            ...
            const datas = await Knex('tb_opendata_menu')
                .select('category')
                .groupBy('category')
                .orderBy('category', 'asc');
    ...
    add_opendata_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Tambah data Iklan aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_opendata_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Update data Iklan aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    delete_opendata_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: data.title,
                deskripsi: 'Delete data Iklan aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_opendata_bantuan_admin: async function (request, h) {
        try {
            const sql = internals_opendata_bantuan.sqlBaseView.clone();
            sql.orderBy('updated_at', 'Desc');
            const data = await sql;
    ...
    add_opendata_bantuan: async function (request, h) {
        const { title, kategori,seksi,content, visibilitas,admin,jenis,komponen } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        // ============ File Upload ============
            ...
            await Knex('tb_opendata_bantuan').insert(addData);
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: title,
                deskripsi: 'Tambah data Bantuan aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_opendata_bantuan: async function (request, h) {
        ...
        try {
            const oldData = await Knex('tb_opendata_bantuan').where({ id }).first();
            if (!oldData) {
                return h.response({ msg: 'Data tidak ditemukan' }).code(404);
            }
            ...
            await Knex('tb_opendata_bantuan').where({ id }).update(updateData);
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: title,
                deskripsi: 'Update data Bantuan aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    delete_opendata_bantuan: async function (request, h) {
        ...
        try {
            const data = await Knex('tb_opendata_bantuan').where({ id }).first();
            if (!data) {
                return h.response({ msg: 'Data tidak ditemukan' }).code(404);
            }
            ...
            await Knex('tb_opendata_bantuan').where({ id }).del();
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: data.title,
                deskripsi: 'Delete data Bantuan aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_opendata_feedback: async function (request, h) {
        try {
            // clone query dasar
            const sql = internals_opendata_feedback.sqlBaseView.clone();
            ...
            console.error('Error in view_opendata_feedback:', err);
            return h
    ...
    delete_opendata_feedback: async function (request, h) {
        const { ids,admin,jenis,komponen } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        try {
            // Cek dulu data yang mau dihapus ada gak (optional)
            const dataList = await Knex('tb_opendata_feedback')
                .whereIn('id', ids)
                .select('feedback');
            const feedbackList = dataList.map((d) => `(${d.feedback})`).join(', ');

            const dataCount = await Knex('tb_opendata_feedback').whereIn('id', ids).count('id as count').first();
            if (dataCount.count === 0) {
                return h.response({ msg: 'Data tidak ditemukan' }).code(404);
            }

            // Hapus data dengan id di dalam array ids
            await Knex('tb_opendata_feedback').whereIn('id', ids).del();

            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: feedbackList, // ← berisi semua feedback
                deskripsi: 'Hapus Feedback masyarakat dari aplikasi Open Data',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_satupeta_menu: async function (request, h) {
        try {
            ...
            const datas = await Knex('tb_satupeta_menu')
                .select('category')
                .groupBy('category')
                .orderBy('category', 'asc');
    ...
    add_satupeta_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Tambah data Iklan aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_satupeta_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Update data Iklan aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    delete_satupeta_iklan: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_satupeta_iklan').where({ id }).del(); await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: data.title,
                deskripsi: 'Delete data Iklan aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_satupeta_bantuan: async function (request, h) {
        try {
            const sql = internals_satupeta_bantuan.sqlBaseView.clone();
            const data = await sql;
            ...
            console.error('Error in view_satupeta_bantuan:', err);
            return h.response({ message: 'Internal Server Error' }).code(500);
        }
    },


    view_satupeta_bantuan_id: async function (request, h) {
        try {
            const { id } = request.params;
            ...
            console.error('❌ Error in view_satupeta_bantuan_id:', err);
            return h.response({ message: 'Internal Server Error' }).code(500);
        }
    },


    add_satupeta_bantuan: async function (request, h) {
        const { title, kategori,seksi,content, visibilitas,admin,jenis,komponen } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        // ============ File Upload ============
            ...
            await Knex('tb_satupeta_bantuan').insert(addData);
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: title,
                deskripsi: 'Tambah data Bantuan aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_satupeta_bantuan: async function (request, h) {
        const id = request.params.id;
        const { title, kategori,seksi,content, visibilitas,admin,jenis,komponen } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        try {
            const oldData = await Knex('tb_satupeta_bantuan').where({ id }).first();
            if (!oldData) {
                return h.response({ msg: 'Data tidak ditemukan' }).code(404);
            }
            ...
            await Knex('tb_satupeta_bantuan').where({ id }).update(updateData);
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: title,
                deskripsi: 'Update data Bantuan aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    delete_satupeta_bantuan: async function (request, h) {
        const id = request.params.id;
        const { admin,jenis,komponen } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');

        try {
            const data = await Knex('tb_satupeta_bantuan').where({ id }).first();
            if (!data) {
                return h.response({ msg: 'Data tidak ditemukan' }).code(404);
            }
            ...
            await Knex('tb_satupeta_bantuan').where({ id }).del();
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: data.title,
                deskripsi: 'Delete data Bantuan aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_satupeta_feedback: async function (request, h) {
        try {
            // clone query dasar
            const sql = internals_satupeta_feedback.sqlBaseView.clone();
            ...
            console.error('Error in view_satupeta_feedback:', err);
            return h
                .response({ message: 'Internal Server Error', error: err.message })
                .code(500);
        }
    ...
    delete_satupeta_feedback: async function (request, h) {
        ...
        try {
            const dataList = await Knex('tb_satupeta_feedback')
                .whereIn('id', ids)
                .select('feedback');
            const feedbackList = dataList.map((d) => `(${d.feedback})`).join(', ');
            // Cek dulu data yang mau dihapus ada gak (optional)
            const dataCount = await Knex('tb_satupeta_feedback').whereIn('id', ids).count('id as count').first();
            if (dataCount.count === 0) {
                return h.response({ msg: 'Data tidak ditemukan' }).code(404);
            }

            // Hapus data dengan id di dalam array ids
            await Knex('tb_satupeta_feedback').whereIn('id', ids).del();
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: feedbackList, // ← berisi semua feedback
                deskripsi: 'Hapus Feedback masyarakat dari aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    add_komponen: async function (request, h) {
        const { title, contents, kategori,title_images_a,title_images_b,title_images_c,linked,admin,jenis,komponen } = request.payload;
        const file = request.payload.file;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
    ...
    update_komponen: async function (request, h) {
        const id = request.params.id;
        const { title, contents,title_images_a,title_images_b,title_images_c, linked,admin,jenis,komponen } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        try {
            ...
            return h.response({ message: 'Komponen berhasil diupdate' }).code(200);
    ...
    view_log: async function (request, h) {
        try {
            const sql = internals_log.sqlBaseView.clone();
            sql.leftJoin('tb_users', 'tb_logging.admin', '=', 'tb_users.id');
            sql.select(
                'tb_logging.*',
                'tb_users.nick as nick_admin',
                'tb_users.role as role_admin'
            );

            const data = await sql.orderBy('tb_logging.updated_at', 'Desc');
