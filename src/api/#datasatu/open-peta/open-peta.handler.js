...
const internals_tb_satupeta_location_geospasial = {
    sqlBaseView: Knex.select('*').from('tb_satupeta_location_geospasial')
};
...
module.exports = {
            ...
            const resultWithUrls = await Promise.all(
                datas.map(async (item) => {
                    ...
                    if (plainItem.images) {
                        try {
                            plainItem.presignedUrl = await generatePresignedUrl( plainItem.images, 60 * 5);
                            plainItem.presignedUrl_Tumb = await generatePresignedUrl(`tumb/` + plainItem.images, 60 * 5);
    ...
    view_item: async function (request, h) {
        ...
        try {
            ...
            const resultkoleksi = await Knex('tb_satupeta_location_maplist')
                ...
                .where('visibilitas','Publik')
                .orderBy('title', 'asc');
            const resultkoleksiGeospasial = await Knex('tb_satupeta_location_maplist')
                .select('id_maplist', 'title')
                .where('tipe', 'Geomap')
                .whereNot('title','Batas Wilayah')
                .where('visibilitas','Publik')
                .orderBy('title', 'asc');
            /* const resultjenisgeospasial = await Knex('tb_satupeta_location_jenis')
            ...
            return h.response({
                ...
                resultlocation,
                resultkoleksi,
                resultkoleksiGeospasial,
                /* resultjenisgeospasial, */
                resultkecamatan,
                resultdesa
            }).code(200);
        }
    ...
    view_data: async function (request, h) {
        ...
        try {
            ...
            const query_bidang = Knex('tb_satupeta_locations')
                ...
                .join('tb_sektor', 'tb_satupeta_locations.sektor_id', '=', 'tb_sektor.id_sektor')
                .distinct(
                    'tb_satupeta_locations.sektor_id',     // DIPAKAI ORDER
                    'tb_sektor.nama_sektor',
                    'tb_opd.nama_opd',
            ...
            const query_kecamatan = Knex('tb_satupeta_location_points')
                ...
                .count('tb_satupeta_location_points.id_location_point as jumlah_point')
                .whereIn('tb_satupeta_location_points.location_id', locationIds)
            ...
            // ---------------------- FILTER ----------------------
            if (search_kecamatan.length > 0) {
                query.whereIn('tb_satupeta_location_points.kecamatan_id', search_kecamatan);
                query_kecamatan.whereIn('tb_satupeta_location_points.kecamatan_id', search_kecamatan);
                query_desa.whereIn('tb_satupeta_location_points.kecamatan_id', search_kecamatan);
            }

            if (search_desa.length > 0) {
                query.whereIn('tb_satupeta_location_points.desa_id', search_desa);
                query_kecamatan.whereIn('tb_satupeta_location_points.desa_id', search_desa);
                query_desa.whereIn('tb_satupeta_location_points.desa_id', search_desa);
            }

            if (queryUrl.search_kunci) {
                query.where('tb_satupeta_location_points.nama_location_point', 'like', `%${queryUrl.search_kunci}%`);
                query_kecamatan.where('tb_satupeta_location_points.nama_location_point', 'like', `%${queryUrl.search_kunci}%`);
                query_desa.where('tb_satupeta_location_points.nama_location_point', 'like', `%${queryUrl.search_kunci}%`);
            }
            ...
            const resultkecamatan = await query_kecamatan.orderBy('tb_kecamatan.nama_kecamatan', 'asc');
            const resultdesa = await query_desa.orderBy('tb_desa.nama_desa', 'asc');
    ...
    view_data_admin: async function (request, h) {
        ...
        try {
            let queryresult = Knex('tb_satupeta_location_points')
                .join('tb_kecamatan', 'tb_satupeta_location_points.kecamatan_id', '=', 'tb_kecamatan.id_kecamatan')
                .join('tb_desa', 'tb_satupeta_location_points.desa_id', '=', 'tb_desa.id_desa')
                .join('tb_satupeta_locations', 'tb_satupeta_location_points.location_id', '=', 'tb_satupeta_locations.id_location')
                .select(
            ...
            if (search_satker?.trim()) {
                queryresult = queryresult.where('tb_satupeta_locations.satker_id', search_satker.trim());
            }
            ...
            if (search_role === 'Super Admin') {
                ...
                querylocation = querylocation
                    .whereIn('tb_satupeta_locations.visibilitas', ['Verified', 'Publik', 'Privat']);
            }
            else if (search_role === 'Operator') {
                querylocation = querylocation
                    .whereIn('tb_satupeta_locations.visibilitas', ['Pending', 'Verified']);
            }
            else if (search_role === 'Verifikator Opd' && search_satker?.trim()) {
                querylocation = querylocation
                    .where('tb_satupeta_locations.satker_id', search_satker.trim())
                    .whereIn('tb_satupeta_locations.visibilitas', ['Draft', 'Pending']);
            }
            else if (search_role === 'Operator Opd' && search_satker?.trim()) {
                querylocation = querylocation
                    .where('tb_satupeta_locations.satker_id', search_satker.trim())
                    .where('tb_satupeta_locations.visibilitas', 'Draft');
            }
            else {
            ...
            let querylocationstatus = Knex('tb_satupeta_locations')
                .join('tb_opd', 'tb_satupeta_locations.satker_id', '=', 'tb_opd.id_opd')
                .join('tb_sektor', 'tb_satupeta_locations.sektor_id', '=', 'tb_sektor.id_sektor')
                .select(
                    'tb_satupeta_locations.*',
            ...
            if (search_satker?.trim()) {
                querylocationstatus = querylocationstatus.where('tb_satupeta_locations.satker_id', search_satker.trim());
            }

            if (search_role === 'Super Admin' || search_role === 'Admin' || search_role === 'Operator') {
                querylocationstatus = querylocationstatus;
            }
            else if ((search_role === 'Operator Opd' || search_role === 'Verifikator Opd') && (search_satker?.trim())) {
                querylocationstatus = querylocationstatus
                    .where('tb_satupeta_locations.satker_id', search_satker.trim());
            }
            else {
            ...
            const resultlocationstatus = await querylocationstatus;
            ...
            if (search_role === 'Super Admin') {
                querylocationmaplist = querylocationmaplist;
            }
            else if (search_role === 'Admin') {
                querylocationmaplist = querylocationmaplist
                    .whereIn('tb_satupeta_location_maplist.visibilitas', ['Verified', 'Publik', 'Privat']);
            }
            else if (search_role === 'Operator') {
                querylocationmaplist = querylocationmaplist
                    .whereIn('tb_satupeta_location_maplist.visibilitas', ['Pending', 'Verified']);
            }
            else if (search_role === 'Verifikator Opd' && search_satker?.trim()) {
                querylocationmaplist = querylocationmaplist
                    .where('tb_satupeta_location_maplist.satker_id', search_satker.trim())
                    .whereIn('tb_satupeta_location_maplist.visibilitas', ['Draft', 'Pending']);
            }
            else if (search_role === 'Operator Opd' && search_satker?.trim()) {
                querylocationmaplist = querylocationmaplist
                    .where('tb_satupeta_location_maplist.satker_id', search_satker.trim())
                    .where('tb_satupeta_location_maplist.visibilitas', 'Draft');
            }
            else {
            ...
            let querylocationmapliststatus = Knex('tb_satupeta_location_maplist')
                .leftJoin('tb_opd', 'tb_satupeta_location_maplist.satker_id', '=', 'tb_opd.id_opd')
                .leftJoin('tb_sektor', 'tb_satupeta_location_maplist.sektor_id', '=', 'tb_sektor.id_sektor')
                .leftJoin('tb_satupeta_locations', 'tb_satupeta_location_maplist.location_id', '=', 'tb_satupeta_locations.id_location')
                .select(
            ...
            if (search_satker?.trim()) {
                querylocationmapliststatus = querylocationmapliststatus.where('tb_satupeta_location_maplist.satker_id', search_satker.trim());
            }

            if (search_role === 'Super Admin' || search_role === 'Admin' || search_role === 'Operator') {
                querylocationmapliststatus = querylocationmapliststatus;
            }
            else if ((search_role === 'Operator Opd' || search_role === 'Verifikator Opd') && (search_satker?.trim())) {
                querylocationmapliststatus = querylocationmapliststatus
                    .where('tb_satupeta_location_maplist.satker_id', search_satker.trim());
            ...
            const resultlocationmapliststatus = await querylocationmapliststatus;

            let queryresultgeospasial = Knex('tb_satupeta_location_geospasial')
                .join('tb_kecamatan', 'tb_satupeta_location_geospasial.kecamatan_id', '=', 'tb_kecamatan.id_kecamatan')
                .join('tb_desa', 'tb_satupeta_location_geospasial.desa_id', '=', 'tb_desa.id_desa')
                .join('tb_satupeta_location_maplist', 'tb_satupeta_location_geospasial.id_maplist', '=', 'tb_satupeta_location_maplist.id_maplist')
                .select(
                    'tb_satupeta_location_geospasial.*',
                    'tb_kecamatan.nama_kecamatan as kecamatan',
                    'tb_desa.nama_desa as desa',
                    'tb_satupeta_location_maplist.title as koleksi_peta',
                    'tb_satupeta_location_maplist.sektor_id as bidangUrusanId'
                )
                .orderBy('tb_satupeta_location_maplist.title', 'asc')
                .orderBy('tb_satupeta_location_geospasial.nama_geospasial', 'asc');

            if (search_satker?.trim()) {
                queryresultgeospasial = queryresultgeospasial.where('tb_satupeta_location_maplist.satker_id', search_satker.trim());
            }

            const resultgeospasial = await queryresultgeospasial;

            let querylocationmaplistgeo = Knex('tb_satupeta_location_maplist')
                .where('tipe', 'Geomap')
                .whereNot('title','Batas Wilayah')
                .orderBy('title', 'asc');

            querylocationmaplistgeo = querylocationmaplistgeo
                .whereIn('visibilitas', ['Publik']);

            if (search_satker?.trim()) {
                querylocationmaplistgeo = querylocationmaplistgeo.where('satker_id', search_satker.trim());
            }
            ...
            const resultlocationmaplistgeo = await querylocationmaplistgeo;
            ...
            return h.response({
                result,
                resultlocation,
                resultlocationstatus,
                resultlocationmaplistUrls,
                resultlocationmapliststatus,
                resultgeospasial,
                resultlocationmaplistgeo,
                resultkecamatan,
                resultdesa
            }).code(200);
    ...
    view_data_admin_result: async function (request, h) {
        const queryUrl = { ...request.query };
        const search_kecamatan = queryUrl.search_kecamatan;
        const search_satker = queryUrl.search_satker;
        const search_location = queryUrl.search_location;
        const search_role = queryUrl.search_role;
        const limit = parseInt(queryUrl.limit) || 2000;
        ...
        try {
            let queryresult = Knex('tb_satupeta_location_points')
                ...
                .orderBy('tb_satupeta_location_points.id_location_point', 'desc')
                .limit(limit); // ✅ ini tambahan

            if (search_location?.trim()) {
                queryresult = queryresult.where(
                    'tb_satupeta_location_points.location_id',
    ...
    view_data_admin_resultlocation: async function (request, h) {
        const queryUrl = { ...request.query };
        const search_kecamatan = queryUrl.search_kecamatan;
        const search_satker = queryUrl.search_satker;
        const search_role = queryUrl.search_role;
    ...
    view_data_admin_resultlocationmaplist: async function (request, h) {
        const queryUrl = { ...request.query };
        const search_kecamatan = queryUrl.search_kecamatan;
        const search_satker = queryUrl.search_satker;
        const search_role = queryUrl.search_role;
    ...
    view_data_admin_geospasial: async function (request, h) {
        const queryUrl = { ...request.query };
        const search_kecamatan = queryUrl.search_kecamatan;
        const search_satker = queryUrl.search_satker;
        const search_role = queryUrl.search_role;
        const search_koleksi = queryUrl.search_koleksi;
        const limit = parseInt(queryUrl.limit) || 2000;
        ...
        try {
            ...
            if (search_koleksi?.trim()) {
                queryresultgeospasial = queryresultgeospasial.where(
                    'tb_satupeta_location_geospasial.id_maplist',
                    search_koleksi.trim()
                );
            }
    ...
    view_data_admin_kecamatan_desa: async function (request, h) {
        const queryUrl = { ...request.query };
        const search_kecamatan = queryUrl.search_kecamatan;
        try {
    ...
    view_data_locationpoint: async function (request, h) {
        const { id } = request.params;

        //log('search_satker', search_satker);

        try {
            const queryresult = Knex('tb_satupeta_location_points')
                .join('tb_kecamatan', 'tb_satupeta_location_points.kecamatan_id', '=', 'tb_kecamatan.id_kecamatan')
                .join('tb_desa', 'tb_satupeta_location_points.desa_id', '=', 'tb_desa.id_desa')
                .join('tb_satupeta_locations', 'tb_satupeta_location_points.location_id', '=', 'tb_satupeta_locations.id_location')
                .select(
                ...
                .where('tb_satupeta_location_points.location_id', id)
                .orderBy('tb_satupeta_location_points.updated_at', 'desc');
            ...
            console.error('❌ Error in view_data_location:', err);
            return h.response({ message: 'Internal Server Error' }).code(500);
        }
    },
    ...
    update_locations: async function (request, h) {
        ...
        const {
            nama_location,satker_id,sektor_id,admin,jenis,komponen,visibilitas
        } = request.payload;
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: oldData.nama_location, // ← berisi semua feedback
                deskripsi: 'Update Lokasi Map Marker dari aplikasi Satu Peta',
                admin,
    ...
    addmulti_locations: async function (request, h) {
        try {
            const { locations,admin,jenis,komponen } = request.payload;
            const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
            ...
            for (const loc of locations) {
                ...
                if (!exists) {
                    ...
                    await Knex('tb_logging').insert({
                        jenis,
                        kategori: komponen,
                        item: loc.nama_location, // ← berisi semua feedback
                        deskripsi: 'Tambah Lokasi Map Marker dari aplikasi Satu Peta',
                        admin,
                        created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                        updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
                    });
    ...
    delete_locations: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: oldData.nama_location, // ← berisi semua feedback
                deskripsi: 'Hapus Lokasi Map Marker dari aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_location_maplist_id: async function (request, h) {
        try {
            ...
            const dataCountRaw = await Knex('tb_satupeta_location_maplist_visitors')
                .where('id_maplist', data.id_maplist)
    ...
    update_location_maplist: async function (request, h) {
        ...
        const {
            ...
            deskripsi, visibilitas, location_id, satker_id, sektor_id, admin,jenis,komponen
        } = request.payload;
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Update Koleksi Peta di aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    addmulti_location_maplist: async function (request, h) {
        try {
            let { locations, admin,jenis,komponen } = request.payload;
            const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
            ...
            for (let i = 0; i < locations.length; i++) {
                ...
                if (!exists) {
                    ...
                    await Knex('tb_logging').insert({
                        ...
                        kategori: komponen,
                        item: loc.title,
                        deskripsi: 'Tambah Koleksi Peta di aplikasi Satu Peta',
                        admin,
                        created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                        updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
                    });
    ...
    delete_location_maplist: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: oldData.title,
                deskripsi: 'Delete Koleksi Peta di aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    add_location_maplist_visitor: async function (request, h) {
        ...
        console.log('maplist:',id_maplist);
    ...
    update_location_point: async function (request, h) {
        ...
        const {
            nama_location_point,coordinat,location_id, kecamatan_id, desa_id,admin,jenis,komponen
        } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: oldData.nama_location_point,
                deskripsi: 'Update Titik Lokasi Peta di aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
            ...
            for (const loc of locations) {
                ...
                if (!exists) {
                    ...
                    await Knex('tb_logging').insert({
                        jenis,
                        kategori: komponen,
                        item: loc.nama_location_point,
                        deskripsi: 'Tambah Titik Lokasi Peta di aplikasi Satu Peta',
                        admin,
                        created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                        updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
                    });
    ...
    delete_location_point: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: oldData.nama_location_point,
                deskripsi: 'Delete Titik Lokasi Peta di aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_datageospasial: async function (request, h) {
        try {
            // Ambil query param
            ...
            const query = Knex('tb_satupeta_location_geospasial');
            query.join('tb_kecamatan', 'tb_satupeta_location_geospasial.kecamatan_id', '=', 'tb_kecamatan.id_kecamatan');
            query.orderBy('tb_satupeta_location_geospasial.nama_geospasial', 'asc');
            //sql.join('tb_desa', 'tb_satupeta_location_points.desa_id', '=', 'tb_desa.id_desa');

            // Filter jika ada
            if (search_kecamatan.length > 0) {
                query.whereIn('kecamatan_id', search_kecamatan);
            }
    ...
    view_datageospasial2: async function (request, h) {
        let search_kecamatan = request.query.search_kecamatan || [];
        let search_desa = request.query.search_desa || [];

        let search_koleksi = request.query.search_koleksi;
        ...
        // ✅ Build query builder terlebih dahulu
            ...
            let query = Knex('tb_satupeta_location_geospasial')
                .join('tb_kecamatan', 'tb_satupeta_location_geospasial.kecamatan_id', '=', 'tb_kecamatan.id_kecamatan')
                .join('tb_desa', 'tb_satupeta_location_geospasial.desa_id', '=', 'tb_desa.id_desa')
                .join('tb_satupeta_location_maplist', 'tb_satupeta_location_geospasial.id_maplist', '=', 'tb_satupeta_location_maplist.id_maplist')
                .select(
                    ...
                    'tb_kecamatan.nama_kecamatan',
                    'tb_desa.nama_desa',
                    'tb_satupeta_location_maplist.title as nama_maplist',
                    'tb_satupeta_location_maplist.periode',
                    'tb_satupeta_location_maplist.tahun_rilis'
                )

                .whereIn('tb_satupeta_location_geospasial.id_maplist', search_koleksi)
                // ORDER BY nama_maplist ASC, nama_geospasial ASC
                //.orderBy('tb_satupeta_location_maplist.title', 'asc')
                .orderBy('tb_satupeta_location_geospasial.updated_at', 'desc');
            if (search_kecamatan.length > 0) {
                query = query.whereIn('tb_satupeta_location_geospasial.kecamatan_id', search_kecamatan);
            }

            // Filter jika ada
            if (search_desa.length > 0) {
                query = query.whereIn('tb_satupeta_location_geospasial.desa_id', search_desa);
            }
            ...
            const features = data.map((row) => ({
                ...
                properties: {
                    ...
                    map_color: row.map_color,
                    nama_geospasial: row.nama_geospasial,
                    luas_area: row.luas_area,
                    satuan: row.satuan,
                    periode: row.periode,
                    tahun_rilis: row.tahun_rilis,
                    nama_maplist: row.nama_maplist,
    ...
    view_geospasial_id: async function (request, h) {
        try {
            const { id } = request.params;
            const sql = internals_tb_satupeta_location_geospasial.sqlBaseView.clone();

            sql.join('tb_kecamatan', 'tb_satupeta_location_geospasial.kecamatan_id', '=', 'tb_kecamatan.id_kecamatan');
            sql.join('tb_desa', 'tb_satupeta_location_geospasial.desa_id', '=', 'tb_desa.id_desa');
            sql.join('tb_satupeta_location_maplist', 'tb_satupeta_location_geospasial.id_maplist', '=', 'tb_satupeta_location_maplist.id_maplist');
            ...
            const data = await sql.where('id_location_geospasial', id).first();
    ...
    update_geospasial: async function (request, h) {

        const { id } = request.params;
        const id_location_geospasial = id;
        const {
            nama_geospasial,geojson,luas_area,satuan,map_color,id_maplist, kecamatan_id, desa_id,admin,jenis,komponen
        } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        try {
            ...
            const oldData = await Knex('tb_satupeta_location_geospasial').where({ id_location_geospasial }).first();
            ...
            await Knex('tb_satupeta_location_geospasial').where({ id_location_geospasial }).update(updateData);
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: oldData.nama_geospasial,
                deskripsi: 'Update Geospasial Peta di aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    add_geospasial: async function (request, h) {
        const {
        ...
        try {
            ...
            const exists = await Knex('tb_satupeta_location_geospasial')
                .where({
                    nama_geospasial,
                    kecamatan_id,
                    desa_id
            ...
            if (!exists) {
                await Knex('tb_satupeta_location_geospasial').insert(addData);
                return h.response({ message: 'Berhasil disimpan' }).code(201);
            }

            return h.response({ msg: 'Data atau Nama Sudah Dipakai' }).code(500);
    ...
    addmulti_geospasial: async function (request, h) {
        try {
            const { locations,admin,jenis,komponen  } = request.payload;
            const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');
            ...
            for (const loc of locations) {
                const exists = await Knex('tb_satupeta_location_geospasial')
                    .where({ nama_geospasial: loc.nama_geospasial, kecamatan_id: loc.kecamatan_id, desa_id: loc.desa_id })
                    .first();

                const addData = {
                    nama_geospasial: loc.nama_geospasial,
                    geojson: loc.geojson,
                    luas_area: loc.luas_area,
                    satuan: loc.satuan,
                    map_color: loc.map_color,
                    id_maplist: loc.id_maplist,
                    kecamatan_id: loc.kecamatan_id,
                    desa_id: loc.desa_id,
                    admin,
                    updated_at: moment().format('YYYY-MM-DD HH:mm:ss'),
                ...
                if (!exists) {
                    ...
                    await Knex('tb_logging').insert({
                        jenis,
                        kategori: komponen,
                        item: loc.nama_geospasial,
                        deskripsi: 'Tambah Geospasial Peta di aplikasi Satu Peta',
                        admin,
                        created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                        updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
                    });
                    ...
                    console.log(`Data atau Nama Sudah Dipakai: ${loc.nama_geospasial}`);
                }
            }
    ...
    addcsv_geospasial: async function (request, h) {
        const { file } = request.payload;
        ...
        try {
            ...
            // Validasi dan persiapkan data untuk insert
            const insertData = [];
            for (const row of rows) {
                // Skip jika geojson kosong atau invalid
                if (!row.geojson) {
                    continue;
                }
                ...
                let geojson;
                try {
                    geojson = typeof row.geojson === 'string'
                        ? JSON.parse(row.geojson)
                        : row.geojson;

                    // Minimal cek format array polygon [[[lng,lat,...],...]]
                    ...
                    console.error('GeoJSON invalid:', row.geojson);
                    continue; // skip row invalid
                }

                // Cek duplikasi berdasarkan nama_geospasial + kecamatan + desa
                const exists = await Knex('tb_satupeta_location_geospasial')
                    .where({
                        nama_geospasial: row.nama_geospasial,
                        kecamatan_id: row.kecamatan,
            ...
            if (insertData.length > 0) {
                await Knex.transaction(async (trx) => {
                    await trx.batchInsert('tb_satupeta_location_geospasial', insertData, 100);
                });
            }
    ...
    delete_geospasial: async function (request, h) {
        const id_location_geospasial = request.params.id; // ambil ID dari URL
        const { admin,jenis,komponen  } = request.payload;
        const tanggal = moment().format('YYYY-MM-DD HH:mm:ss');

        try {
            const oldData = await Knex('tb_satupeta_location_geospasial')
                .where({ id_location_geospasial })
                .first();
            ...
            await Knex('tb_satupeta_location_geospasial')
                .where({ id_location_geospasial })
                .del();
            await Knex('tb_logging').insert({
                jenis,
                kategori: komponen,
                item: oldData.nama_location_point,
                deskripsi: 'Delete Geospasial Peta di aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    view_list_artikel: async function (request, h) {
        try {
            ...
            query.where('visibilitas', 'like', `Publik`);
            // ambil data + total
    ...
    add_artikel: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: title,
                deskripsi: 'Tambah data Artikel aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    update_artikel: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: oldData.title,
                deskripsi: 'Update data Artikel aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
    ...
    delete_artikel: async function (request, h) {
        ...
        try {
            ...
            await Knex('tb_logging').insert({
                ...
                kategori: komponen,
                item: oldData.title,
                deskripsi: 'Delete data Artikel aplikasi Satu Peta',
                admin,
                created_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss'),
                updated_at: tanggal || moment().format('YYYY-MM-DD HH:mm:ss')
            });
